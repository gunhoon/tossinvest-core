...
class TestTossInvestServices(unittest.TestCase):
    def setUp(self):
        ...
        self.client._access_token = "mock_token"
        self.client._token_expires_at = time.time() + 1000

    @patch("requests.Session.request")
    def test_market_service(self, mock_request):
        ...
        self.client.market.get_prices(["005930", "000660"])
        ...
        # Test get_candles
        self.client.market.get_candles("005930", "1d", count=10, adjusted=True)
        mock_request.assert_called_with(
            "GET",
        ...
        self.client.market.get_exchange_rate(base_currency="USD", quote_currency="KRW", date_time="2026-06-20T12:00:00Z")
        mock_request.assert_called_with(
            "GET",
    ...
    @patch("requests.Session.request")
    def test_account_service(self, mock_request):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"mock": "data"}
        mock_request.return_value = mock_response
        ...
        self.client.account.get_holdings(account_seq=456)
        mock_request.assert_called_with(
            "GET",
            "https://openapi.test.com/api/v1/holdings",
            headers={
    ...
    @patch("requests.Session.request")
    def test_order_service_placement(self, mock_request):
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        self.client.order.create_quantity_order(
            symbol="005930",
            side="buy",
            order_type="limit",
            quantity=10,
        ...
        self.client.order.create_amount_order(
            symbol="AAPL",
            side="sell",
            order_amount=150.50,
    ...
    @patch("requests.Session.request")
    def test_order_service_actions_and_queries(self, mock_request):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"mock": "data"}
        mock_request.return_value = mock_response
        ...
        self.client.order.cancel_order("ord_999")
        mock_request.assert_called_with(
            "POST",
